...
public final class TimeUtils {
    ...
    public static long getMillis(final long millis) {
        return getUtcMillis(millis) + SLog.getSettings().getZoneOffset().getValue();
    }
    ...
    public static String getCurTime() {
        return format(getCurMillis(), SLog.getSettings().getTimeFormat());
    }

    /**
     * 获取目标时区的当前时间（默认为东八区）.添加毫秒
     *
     * @return 目标时区的当前时间
     */
    public static String getCurTimeLimitMs() {
        SimpleDateFormat sdFormat = new SimpleDateFormat(SLog.getSettings().getTimeFormat());
        return sdFormat.format(getCurMillis());
    }
